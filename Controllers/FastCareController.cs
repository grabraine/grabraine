...
namespace WebApi.Controllers
{
    ...
    public class FastCareController : Controller
    {
       private FastCareContext _context; 
       private HKMasterContext _hkcontext;
       private ScheduleContext _contextSched;
       private ArisContext _context2;
   ...
   public FastCareController(FastCareContext context, ArisContext context2, HKMasterContext hkcontext,ScheduleContext  contextSched)
      ...
      [Route("GetAllSubscriber")]
       ...
       [HttpGet("GetSubscriber2")]
    public IActionResult GetSubscriber2(string id)
        {
         List<Subscribers> Res = new List<Subscribers>(); 
       
           var model = from s in  _context.Subscribers.Where(x => x.SystemNoteKey == id)
            select s;
            var model2 = from s in  _context2.Subscribers.Where(x => x.SystemNoteKey == id)
            select s;
                var model3 = from s in  _context2.Subscribers.Where(x => x.SystemNoteKey == id)
            select s;
           
            
             var Result = model.ToList();
               var Result2 = model2.ToList();
      var final = Result.Concat(Result2);
            return Json(final);
